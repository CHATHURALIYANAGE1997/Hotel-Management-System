...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty($_POST["guest_name"])) {
    ...
    if (empty($_POST["bill"])) {
		$billErr="Billing_Address is required.";
  } else {
  ...
  if(empty($Guest_iderr)){
        ...
        $sql = "SELECT Guest_Id FROM company WHERE Guest_Id = :guest_id";
  ...
  if(empty($guest_nameErr) && empty($guest_idErr) &&!empty($guest_id) && empty($billErr)&& !empty($guest_name) && !empty($bill)  ){
    try{$sql14="UPDATE company SET Billing_Address='$bill',C_Name='$guest_name' WHERE Guest_Id=$guest_id";
