...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
  if (empty($_POST["con1"])) {
    ...
    if (!preg_match("/^[0-9]*$/",$con1)) {
	  ...
	  $con2Err = "contact 1 is required";
  } else {
    $con2 = test_input($_POST["con2"]);
    // check if name only contains letters and whitespace
    if (!preg_match("/^[0-9]*$/",$con1)) {
  ...
  if(empty($con1Err)&& empty($con2Err) &&empty($guest_idErr) && !empty($guest_id) && !empty($con1) && !empty($con2) ){
    try{
		
		$sql4 = "UPDATE contact SET Contact_No='$con2' WHERE Guest_Id=$guest_id AND Contact_No=$con1";
         // use exec() because no results are returned
        
        

       if($pdo->exec($sql4)){echo "New record created successfully. Last inserted ID is: " .$guest_id;}
...
<div class="demo-table">
  ...
  Old contact Number: <input type="text" name="con1" class="demo-input-box" >
  <span class="error-message ">* <?php echo $con1Err;?></span><br> 
  New contact Number: <input type="text" name="con2" class="demo-input-box" >
